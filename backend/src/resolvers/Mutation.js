...
const Mutations = {
  ...
  updateReservation(parent, args, ctx, info) {
    // take a copy of the updates
    const updates = { ...args };
    // remove the ID from the updates
    delete updates.id;
    // run the update method
    return ctx.db.mutation.updateReservation(
      {
        data: updates,
        where: {
          id: args.id
  ...
  async deleteReservation(parent, args, ctx, info) {
    ...
    const ownsReservation = reservation.user.id === ctx.request.userId;
    if (!ownsReservation) {
      throw new Error("Sorry, it's not your reservation");
    }
    // delete it
